...
class OpenAIAgentExecutor(AgentExecutor):
    ...
    def __init__(
        ...
        if self.provider == 'groq':
            selected_api_key = os.getenv('GROQ_API_KEY', api_key)
            selected_base_url = os.getenv('GROQ_BASE_URL', 'https://api.groq.com/openai/v1')
            self.model = os.getenv('GROQ_MODEL', 'llama-3.3-70b-versatile')
            self.max_tokens = int(os.getenv('GROQ_MAX_TOKENS', '1024'))
            fallback_models = os.getenv('GROQ_MODEL_FALLBACKS', self.model)
        else:
            selected_api_key = os.getenv('GEMINI_API_KEY', api_key)
            ...
            fallback_models = os.getenv('GEMINI_MODEL_FALLBACKS', self.model)
    ...
    def _latest_success_result(
        self,
        execution_log: dict[str, Any],
        step_name: str,
    ) -> dict[str, Any] | None:
        for step in reversed(execution_log.get('steps', [])):
            if step.get('step') != step_name or step.get('status') != 'success':
                continue
    ...
    def _normalize_tool_args(
        ...
        if function_name == 'data_fetcher':
            normalized_args['source'] = normalized_args.get('source', 'sales_data.csv')

        if function_name == 'data_transformer':
            fetch_result = self._latest_success_result(execution_log, 'data_fetcher') or {}
            latest_data_id = fetch_result.get('data_id')
            if latest_data_id:
                normalized_args['data_id'] = latest_data_id
            normalized_args['region'] = normalized_args.get('region', 'all') or 'all'

        if function_name == 'chart_generator':
            transform_result = self._latest_success_result(execution_log, 'data_transformer') or {}
            latest_tx_id = transform_result.get('transformed_data_id')
            if latest_tx_id:
                normalized_args['transformed_data_id'] = latest_tx_id
            normalized_args['chart_type'] = normalized_args.get('chart_type', 'bar') or 'bar'

        if function_name == 'report_composer':
            chart_result = self._latest_success_result(execution_log, 'chart_generator') or {}
            latest_chart_id = chart_result.get('chart_id')
            if latest_chart_id:
                normalized_args['chart_id'] = latest_chart_id
            normalized_args['summary_text'] = normalized_args.get(
                'summary_text',
                'Auto-generated report summary.',
            )

        if function_name == 'email_dispatcher':
            report_result = self._latest_success_result(execution_log, 'report_composer') or {}
            latest_report_id = report_result.get('report_id')
            if latest_report_id:
                normalized_args['report_id'] = latest_report_id
            normalized_args['recipient'] = normalized_args.get(
                'recipient',
                'ops@example.com',
            )

        if function_name == 'get_chart':
            chart_result = self._latest_success_result(execution_log, 'chart_generator') or {}
            latest_chart_id = chart_result.get('chart_id')
            requested_chart_id = str(normalized_args.get('chart_id', '')).strip()
            if latest_chart_id and requested_chart_id in {'', 'CHART_ID', 'chart_id', 'latest'}:
                normalized_args['chart_id'] = latest_chart_id

        if function_name == 'get_report':
            report_result = self._latest_success_result(execution_log, 'report_composer') or {}
            latest_report_id = report_result.get('report_id')
            requested_report_id = str(normalized_args.get('report_id', '')).strip()
            if latest_report_id and requested_report_id in {'', 'REPORT_ID', 'report_id', 'latest'}:
                normalized_args['report_id'] = latest_report_id
    ...
    async def _execute_llm_tool_call(
        ...
        payload, step_log = await self._call_tool_step(function_name, normalized_args)
                ...
                [
                    TextPart(
                        text=(
                            ...
                            'Set GROQ_API_KEY when LLM_PROVIDER=groq, '
                            'or GEMINI_API_KEY when LLM_PROVIDER=gemini.'
                        )
                    )
                ]
            )
